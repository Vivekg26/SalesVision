# To run this -->>>>>  arch -x86_64 python3 app.py
...
# CORS(app, resources={r"/save": {"origins": "http://localhost:3000"}})
...
def prophet_forecast(daily_sales):
    ...
    df = pd.DataFrame({'ds': pd.date_range(
        start='2022-01-01', periods=365), 'y': input_list})
...
def generate_forecast(daily_sales):
    ...
    model = ExponentialSmoothing(
        df.sales, trend='add', seasonal='add', seasonal_periods=7).fit()
    ...
    pred = model.predict(
        start=df.sales.index[-20], end=df.sales.index[-1]).astype(int).tolist()
...
def save_to_mongodb():
    ...
    response = jsonify({"success": "Uploaded"})
...
def process_array():
    ...
    # Assuming the array is passed in the 'array' field
    input_array = input_data['options']
